...
plot_sNMF_bars <- function(a, group, minK, miss_ind) {
  ...
  qmatrix <- read.csv(file = here(paste0("analyses/sNMF_runs/qmatrices/", group, "_mi", miss_ind, "_a", a, ".csv")), header = TRUE)
  
  ## Best-fit K:
  K <- length(unique(qmatrix$cluster_assign))
  
  ## Load plot order:
  plot_order <- read.csv(file = here(paste0("analyses/sNMF_runs/samples/order_", group, "_mi", miss_ind, "_a", a, ".csv")), header = TRUE)
  ...
  ## Labels:
  qmatrix$ID_label <- qmatrix$ID
  qmatrix$ID_label <- gsub(qmatrix$ID_label, pattern = "_Ct_[a-z]+", replacement = "")
  qmatrix$ID_label <- gsub(qmatrix$ID_label, pattern = "_ct_[a-z]+", replacement = "")
  qmatrix$ID_label <- gsub(qmatrix$ID_label, pattern = "NA_([A-Z]+)([0-9]+)", replacement = "\\1_\\2")
  qmatrix$ID_label <- gsub(qmatrix$ID_label, pattern = "_", replacement = " ")
  
  ## Set factors levels to preserve order in plots:
  qmatrix$ID_label <- factor(qmatrix$ID_label, levels = unique(qmatrix$ID_label))
  ...
  plot <- ggplot(data = qmatrix_m, aes(y = ID_label)) +
...
plot_maps <- function(a, group, minK, miss_ind) {
  ...
  ## Save:
  write.csv(label_df, file = here(paste0("analyses/sNMF_runs/samples/labels_", group, "_mi", miss_ind, "_a", a, ".csv")), row.names = FALSE, quote = FALSE)
  ...
  plot <- ggplot() +
    ...
    theme(panel.background = element_rect(fill = "aliceblue"),
          ...
          strip.text = element_text(size = 7, face = "italic", margin = margin(t = 0, r = 0, b = 2, l = 0)),
...
plot_tree <- function(a, group, miss_ind) {
  ...
  assignments <- read.csv(file = here(paste0("analyses/sNMF_runs/assignments/", group, "_mi", miss_ind, "_a", a, ".csv")), header = TRUE)
...
run_PCA <- function(a, group, miss_ind) {
  ...
  geno <- here(paste0("analyses/sNMF_runs/data/", group, "_mi", miss_ind, ".geno"))
  ...
  ## Loading assigments:
  assignments <- read.csv(header = TRUE, file = here(paste0("analyses/sNMF_runs/assignments/", group, "_mi", miss_ind, "_a", a, ".csv")))
  ...
  ## Using a phylogeny to order samples in structure plots:
  full_tree <- read.nexus(here("analyses/RAxML/Ctenotus.nex"))
...
plot_PCA <- function(a, group, miss_ind) {
  ...
  unlink(here("analyses/sNMF_runs/data/*.lfmm"))
  unlink(here("analyses/sNMF_runs/data/*.pca"), recursive = TRUE)
...
## Function:
run_group <- function(group, miss_ind, project) {

  ## Testing:
  #group <- "atlas_gr" ;  miss_ind <- 0.5 ;  a <- 200 ;  project <- "previous"
  
  ## alpha parameter (established in preliminary runs):
  ...
  ## Plot annotation: 
  plot_annotation(tag_levels = "A", title = paste0("Ctenotus ", gsub(x = group, pattern = "_gr", replacement = ""), " group")) &
  theme(plot.tag = element_text(size = 18),
        plot.tag.position = c(-0.010, 1),
        plot.title = element_text(size = 14, hjust = 0.5, face = "italic", margin = margin(t = 10, r = 0, b = 10, l = 0)))
  
  ## Saving plot:
  ggsave(plot = plot_f, width = 18, height = 22, units = "cm", limitsize = FALSE, device = "pdf",
         filename = here(paste0("analyses/sNMF_runs/plots/", group, "_mi", miss_ind, "_a", a, ".pdf")))
...
cpu <- 32
rep <- 50
project <- "previous" ## Change to "new" if starting the genotypic clustering analyses over again.
...
## Run:
purrr::map(groups, run_group, miss_ind = 0.5, project = project)
...
join_assignments <- function(group, miss_ind) {
  ...
  ## Load assignments:
  assignments <- read.csv(header = TRUE, file = here(paste0("analyses/sNMF_runs/assignments/", group, "_mi", miss_ind, "_a", a, ".csv")), stringsAsFactors = FALSE)
  ...
  label_df <- read.csv(file = here(paste0("analyses/sNMF_runs/samples/labels_", group, "_mi", miss_ind, "_a", a, ".csv")), header = TRUE, stringsAsFactors = FALSE)
  label_df$OTU <- paste0(group, "_", label_df$cluster)
  
  ## Save labels:
  write.csv(label_df, file = here(paste0("analyses/sNMF_runs/samples/labels_", group, "_mi", miss_ind, "_delimited_OTUs.csv")), row.names = FALSE, quote = FALSE)
...
## Save assignments:
write.csv(assign_df, file = here("analyses/outputs/assignments_Ctenotus_OTUs.csv"), row.names = FALSE, quote = FALSE)
