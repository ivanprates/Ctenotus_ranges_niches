...
run_sNMF <- function(mid, ms, a){
  ...
  plot(y = crossEntropy, x = CEnames, cex = 1.2, col = "blue", pch = 19)
  ...
  ## Extracting qmatrix for every K:
  for (K in minK:maxK){
...
plot_sNMF_bars <- function(mid, ms, a){
  ...
  K <- sNMF_results$bestK
    ...
    scale_fill_manual("sNMF Cluster", values = cluster_palette) + 
    ...
    ## Saving plot:
    save_plot(plot = sNMF_plot, base_width = 30, base_height = 7,
              filename = paste0(here(), "/LEA/", ipyrad_run, "/mac" , mac, "/sNMF_plots/mid", mid, "_ms", ms, 
                                "/mid", mid, "_ms", ms, "_a", a, "_e", crossEntropy, "_K", K, ".pdf"))
...
plot_maps <- function(mid, ms, a){
  ...
  K <- sNMF_results$bestK
    ...
    map_clusters <- ggplot() +
      ...
      labs(x = "", y = "") +
      facet_wrap(~cluster_assign, ncol = 6) + 
    ...
    ## Saving plot:
    save_plot(plot = map_clusters, base_width = 16, base_height = 10,
              filename = paste0(here(), "/LEA/", ipyrad_run, "/mac" , mac, "/sNMF_maps/mid", mid, "_ms", ms, 
                                "/mid_", mid, "_ms", ms, "_a", a, "_e", crossEntropy, "_K", K, "_map.pdf"))
...
r <- 10 ## Number of replicates for each K value.
cpu <- 8 ## Number of threads to use.
ipyrad_run <- "inornatus_gr_c90_ni281_mi070" ## Output files from ipyrad.
minK <- 4 ## Lowest number of populations tested.
maxK <- 24 ## Highest number of populations tested.
...
for (mac in c(2, 3, 4)){
  ...
  for (mid in c(0.8, 0.85)){ ## Level of missing data per sample.
